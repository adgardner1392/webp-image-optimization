...
class WebP_Image_Optimization
{
    ...
    public function register_settings()
    {
        ...
        add_settings_field(
            'dont_convert_jpeg',
            __('Disable conversion of JPEG images.', 'webp-image-optimization'),
        ...
        add_settings_field(
            'dont_convert_png',
            __('Disable conversion of PNG images.', 'webp-image-optimization'),
        ...
        add_settings_field(
            'conversion_format',
            __('Conversion Format', 'webp-image-optimization'),
            array($this, 'conversion_format_render'),
            'webp_image_optimization_settings',
            'webp_image_optimization_settings_section'
        );
    }
    ...
    public function sanitize_settings($input)
    {
        ...
        // Sanitize conversion_format
        $allowed_formats = array('webp', 'avif');
        $output['conversion_format'] = (isset($input['conversion_format']) && in_array($input['conversion_format'], $allowed_formats, true))
            ? $input['conversion_format']
            : 'webp';
...
?>
    ...
    /**
     * Render Conversion Format radio buttons
     */
    public function conversion_format_render()
    {
        $options = get_option('webp_image_optimization_settings');
        $format = isset($options['conversion_format']) ? $options['conversion_format'] : 'webp';
    ?>
        <fieldset>
            <label>
                <input type="radio" name="webp_image_optimization_settings[conversion_format]" value="webp" <?php checked('webp', $format); ?> />
                <?php esc_html_e('WebP', 'webp-image-optimization'); ?>
            </label><br>
            <label>
                <input type="radio" name="webp_image_optimization_settings[conversion_format]" value="avif" <?php checked('avif', $format); ?> />
                <?php esc_html_e('AVIF', 'webp-image-optimization'); ?>
            </label>
        </fieldset>
    <?php
    }
...
<?php
    ...
    public function generate_attachment_metadata($metadata, $attachment_id)
    {
        ...
        $conversion_type = get_option('webp_image_optimization_settings')['conversion_format'];
        ...
        if ($conversion_type === 'webp') {
        ...
        } elseif ($conversion_type === 'avif') {
            // Convert to avif
            $avif_file = $this->convert_to_avif($file);
            ...
            // Update the attachment to point to the avif file
            $upload_dir = wp_upload_dir();
            $relative_avif_path = str_replace($upload_dir['basedir'] . '/', '', $avif_file);
            $avif_url = $upload_dir['baseurl'] . '/' . $relative_avif_path;

            // Update the attachment's '_wp_attached_file' meta to point to avif
            ...
            // Regenerate the metadata based on the new avif file
