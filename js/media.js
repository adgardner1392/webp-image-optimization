...
document.addEventListener('DOMContentLoaded', function () {
     ...
     */
    document.addEventListener('click', function (e) {
        var button = e.target.closest('.convert-to-webp');
        if (!button) return;
        ...
        // Disable button + store original text
        button.disabled = true;
    ...
    /**
     * Handle "Convert to AVIF" button click
     */
    document.addEventListener('click', function (e) {
        var button = e.target.closest('.convert-to-avif');
        if (!button) return;
        ...
        // Prepare request
        var data = new URLSearchParams();
        data.append('action', 'avif_convert_attachment'); // matches our new wp_ajax_ hook
        data.append('nonce', webpImageOptimization.nonce);
        data.append('attachment_id', attachmentId);
        ...
        .then(function (response) {
            if (response.success) {
                alert('Image successfully converted to AVIF.');
                location.reload();
            } else {
                alert('Conversion failed: ' + response.data);
                button.disabled = false;
